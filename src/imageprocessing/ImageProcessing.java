...
public class ImageProcessing {
	...
	public ImageProcessing(TwinView views) {
		...
		// TODO add here further image processing objects (they are inserted
		// into the Image menu)
	}
		...
		for (int y = 0; y < outData.height; y++) {
			...
			for (int x = 0; x < outData.width; x++) {
				...
				if (imageType == PicsiSWT.IMAGE_TYPE_GRAY) {
					...
					RGB cA = inData.palette.getRGB(A), cB = inData.palette.getRGB(B), cC = inData.palette.getRGB(C), cD = inData.palette.getRGB(D);

					RGB cG = new RGB(ImageProcessing.clamp8(a0 * b0 * cA.red + a1 * b0 * cB.red + a0 * b1 * cC.red + a1 * b1 * cD.red), ImageProcessing.clamp8(a0 * b0 * cA.green
							+ a1 * b0 * cB.green + a0 * b1 * cC.green + a1 * b1 * cD.green), ImageProcessing.clamp8(a0 * b0 * cA.blue + a1 * b0 * cB.blue + a0 * b1 * cC.blue + a1
							* b1 * cD.blue));
